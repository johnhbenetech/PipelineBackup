serveAPI = function(terms){
  ...
  Posts.find(parameters.find, parameters.options).forEach(function(post) {
    ...
    var postOutput = {
      ...
      impact: post.impact,
      need: post.need,
      missionfit: post.missionfit,
      legality: post.legality,
      currentmoney: post.currentmoney,
      futuremoney: post.futuremoney,
      cost: post.cost,
      talent: post.talent,
      technicalrisk: post.technicalrisk,
      internalchampion: post.internalchampion,
      communityofusers: post.communityofusers,
      communityofpartners: post.communityofpartners,
      portfoliofit: post.portfoliofit,
      exitstrategies: post.exitstrategies,
      existingdata: post.existingdata,      
